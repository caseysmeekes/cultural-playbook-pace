...
import { getCountryIntelligence, getSourceStatus } from '../data/decisionEngine'
...
export default function PillarDetail({ pillar, country, activity, industry }) {
  ...
  const countryText = intelligence.pace[pillar]
  ...
  return <div className={`pillar-detail detail-${pillar}`}>
    <div className="detail-intro">
      ...
      <div className="detail-country"><span>{country.name}</span><small>{activity} · {industry}</small><em>{getSourceStatus(country)}</em></div>
    </div>
    <div className="iat-grid">
      <article><span className="iat-number">01</span><b>UNDERSTAND</b><h4>The cultural lens</h4><p>{guide.meaning}</p><p className="country-note"><strong>{country.name}:</strong> {countryText}</p></article>
    ...
    <style jsx>{`
      ...
      .detail-country{text-align:right;align-self:center}.detail-country span{display:block;font-weight:800;font-size:12px}.detail-country small{display:block;color:#7b8795;font-size:9px;margin-top:4px}.detail-country em{display:block;color:#4b7d62;font-size:8px;font-style:normal;font-weight:800;margin-top:5px;letter-spacing:.04em}
